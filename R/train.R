...
#' @param path.validation Path to folder where individual or multiple fasta files are located for validation
...
#' @param tensorboard.log path where tensorboard server looks for log files, should end with backslash
#' @export
trainNetwork <- function(path,
  ...
  if (missing(dataset)) {
    ...
    if (missing(labels)) {
      ...
      gen.validation <-
        fastaFileGenerator(corpus.dir = path.validation, batch.size = batch.size, maxlen = maxlen)
    } else {
    ...
    history <-
      model %>% keras::fit_generator(
        generator = gen,
        validation_data = gen.validation,
        validation_steps = 1,
