writeStatesToH5 <- function(model.path, layer_depth, fasta.path, round_digits = 2, h5_filename,
  ...
  h5_file[["fasta_file"]] <- fasta.path
  h5_file[["start_indices"]] <- start_indices
  h5_file[["states"]] <- array(0, dim = c(0, layer.size)) 
  writer <- h5_file[["states"]]
  
  # how often to call the generator function
  numberOfGeneratorCalls <- ceiling(length(start_indices)/batch.size)
  
  # generator for input 
  gen <- fastaFileGenerator(corpus.dir = fasta.path, batch.size = batch.size, maxlen = maxlen, vocabulary = vocabulary, 
                             step = step, seqStart = seqStart, withinFile = withinFile, seqEnd = seqEnd, genPredictions = TRUE)
...
readRowsFromH5 <- function(h5_path, rows){
  h5_file <- hdf5r::H5File$new(h5_path, mode = "r")
  read_lines <- h5_file[["states"]]
...
# extractStartIndices <- function(fasta.path, sequence, maxlen, step, vocabulary, seqStart, withinFile, seqEnd, considerTargets = FALSE){
#   
#   if (!missing(sequence)){
#     seq <- sequence
#   } else {  
...
#   
#   len_seq <- nchar(seq)
#   if (considerTargets) maxlen <- maxlen + 1
